...
const Index = () => {
  ...
  return <div className="min-h-screen flex flex-col dark bg-netflix-bg bg-gradient-to-b from-netflix-bg to-netflix-dark">
      ...
      <main className="flex-1 container pt-4 pb-20 md:pb-6 animate-fade-in py-[19px] px-[17px]">
        <section className="mb-8">
          ...
          duration: 0.6
            ...
            duration: 0.5
              ...
              <Link to="/codigos" className="text-law-accent hover:underline font-medium flex items-center gap-1 transition-all hover:text-law-accent/80">
                Explorar todos os códigos
              </Link>
            </motion.div>
          </motion.div>
                    ...
                    <Card className={`bg-netflix-dark border-gray-800 h-full shadow-lg hover:shadow-xl transition-all duration-300 ${category.borderColor} border overflow-hidden`}>
                      <CardContent className="p-4 md:p-6 flex flex-col items-center text-center h-full">
                        <div className={`p-3 rounded-full ${category.bgColor} ${category.borderColor} border my-2 md:my-3 shadow-glow-sm relative`}>
                          ...
                          {category.badge && <span className="absolute -top-2 -right-2 bg-netflix-red text-white text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center">
                              {category.badge}
                            </span>}
                        </div>
        ...
        {/* Recent Codes */}
        {recentVisitedCodes.length > 0 && <motion.section initial={{
        opacity: 0,
        y: 30
      }} animate={{
        opacity: 1,
        ...
        delay: 0.6
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {recentVisitedCodes.map((code, index) => code && <motion.div key={code.id} initial={{
            opacity: 0,
            x: -20
          }} animate={{
            opacity: 1,
            ...
            delay: 0.1 * index
                    ...
                    <Link to={`/codigos/${code.id}`} className="block p-3 bg-netflix-dark border border-gray-800 rounded-md hover:bg-netflix-dark/70 transition-colors">
        ...
        <section>
          ...
          delay: 0.4
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {legalCodes.slice(0, 9).map((code, index) => <motion.div key={code.id} initial={{
              opacity: 0,
              y: 20
            }} animate={{
              opacity: 1,
              ...
              transition: {
                duration: 0.2
                  ...
                  <Link to={`/codigos/${code.id}`} className="p-4 bg-gradient-to-br from-netflix-dark to-netflix-bg border border-gray-800 rounded-lg shadow-md hover:shadow-xl transition-all duration-300 flex flex-col h-full">
                    <div className="flex items-center mb-2">
                      {code.category === 'código' ? <Scale className="mr-2 h-5 w-5 text-law-accent" /> : code.category === 'estatuto' ? <Gavel className="mr-2 h-5 w-5 text-netflix-red" /> : <div className="w-5 h-5 mr-2"></div>}
            ...
            delay: 1
              ...
              <Link to="/codigos" className="text-law-accent hover:text-law-accent/80 underline underline-offset-4 transition-colors hover:scale-105 inline-flex items-center gap-1">
                Ver todos
              </Link>
            </motion.div>
          </motion.div>
