...
const Index = () => {
  ...
  // Get recent codes
  const recentVisitedCodes = recentCodes
    .map(codeId => legalCodes.find(code => code.id === codeId))
    .filter(Boolean)
    .slice(0, 3);
        ...
        <section className="mb-8">
          ...
          <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-2 md:grid-cols-5 gap-3 sm:gap-4 mb-12">
            {categories.map((category) => {
              ...
              return (
                <motion.div key={category.id} variants={item} whileHover={{ scale: 1.03, transition: { duration: 0.2 }}} className="col-span-1">
