      ...
      <main className="flex-1 container py-6 pb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            return <button key={category} onClick={() => setActiveFilter(activeFilter === category ? null : category)} className={`px-3 py-1.5 rounded-md text-sm font-medium flex items-center gap-1
        ...
        {Object.keys(groupedCodes).length > 0 ? Object.entries(groupedCodes).map(([category, codes]) => {
        ...
        return <div key={category} className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {codes.map(code => <Link key={code.id} to={`/codigos/${code.id}`} className="p-6 bg-netflix-dark border border-gray-800 rounded-lg shadow-md hover:shadow-lg transition-all duration-300 hover:scale-105 card-hover-effect">
