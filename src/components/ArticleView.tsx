...
export const ArticleView = ({
  ...
  const hasAudioComment = article.comentario_audio && article.comentario_audio.trim() !== '' && (article.comentario_audio.startsWith('http') || article.comentario_audio.startsWith('data:'));
  ...
  const toggleAudioPlay = () => {
    ...
    if (isCurrentPlaying) {
      if (isPlaying) {
        ...
        playAudio(article.id, article.comentario_audio!).catch(error => {
  ...
  const renderDialog = () => {
    ...
    switch (activeDialog) {
      ...
      case 'comment':
        ...
        content = hasAudioComment ? 'Ouça o comentário de áudio sobre este artigo.' : 'Este artigo ainda não possui comentário em áudio.';
    ...
    return <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
        <div className="relative w-full max-w-md max-h-[90vh] overflow-y-auto rounded-lg bg-background-dark border border-gray-700 shadow-xl animate-in slide-in-from-bottom-10 duration-300">
          ...
          <div className="p-4 text-sm text-gray-300 space-y-3">
            {activeDialog === 'comment' ? hasAudioComment ? <div className="flex flex-col items-center justify-center gap-4 my-4">
                  <Button variant="outline" size="lg" disabled={isAudioLoading} className={`rounded-full h-16 w-16 p-0 ${isPlaying ? 'bg-law-accent text-white' : 'bg-gray-800 text-law-accent'}`} onClick={toggleAudioPlay}>
                    {isAudioLoading ? <Loader2 className="h-6 w-6 animate-spin" /> : isPlaying ? <VolumeX className="h-6 w-6" /> : <Volume className="h-6 w-6" />}
                  </Button>
                  <p className="text-center text-sm">
                    {isAudioLoading ? "Carregando..." : isPlaying ? "Reproduzindo..." : "Clique para ouvir"}
                  </p>
                </div> : <p className="text-center py-8 text-gray-400">Comentário em áudio não disponível para este artigo.</p> : content.split('\n').map((paragraph, i) => <p key={i} className="leading-relaxed">{paragraph}</p>)}
          </div>
        </div>
      </div>;
  };
  return <article id={`article-${article.id}`} className={cn("legal-article bg-background-dark p-4 rounded-md border mb-6 transition-all hover:shadow-md relative group", hasAudioComment ? "border-gray-600 hover:border-law-accent/50" : "border-gray-800 hover:border-gray-700")}>
      ...
      <div className={cn("legal-article-content whitespace-pre-line mb-3", !hasNumber && "text-center bg-red-500/10 p-3 rounded", "font-serif text-base md:text-lg leading-relaxed")}>
        {contentLines.map((line, index) => <p key={index} className="mb-4 leading-relaxed">{line}</p>)}
      </div>
