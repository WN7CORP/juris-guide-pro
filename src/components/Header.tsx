  ...
  return <header className="sticky top-0 z-20 bg-netflix-bg border-b border-gray-800">
      <div className="container mx-auto flex items-center justify-between py-4">
          ...
          <NavigationMenu>
            <NavigationMenuList>
              {menuItems.map(item => <NavigationMenuItem key={item.path}>
                  <Link to={item.path} className={cn("flex items-center px-4 py-2 text-sm font-medium transition-colors rounded-md", currentPath === item.path ? "text-netflix-red" : "text-gray-300 hover:text-netflix-red", item.highlight && currentPath !== item.path ? "animate-pulse" : "")}>
                    <item.icon className="mr-2 h-4 w-4" />
                    {item.label}
                    {item.highlight && currentPath !== item.path && <span className="ml-1 h-2 w-2 rounded-full bg-netflix-red"></span>}
                  </Link>
                </NavigationMenuItem>)}
            </NavigationMenuList>
          </NavigationMenu>
        ...
        <nav className="flex md:hidden items-center space-x-1 sm:space-x-4">
          {menuItems.map(item => <Link key={item.path} to={item.path} className={cn("px-2 py-1 text-sm font-medium transition-colors rounded-md flex flex-col items-center", currentPath === item.path ? "text-netflix-red" : "text-gray-300 hover:text-netflix-red", item.highlight && currentPath !== item.path ? "animate-pulse" : "")}>
              <item.icon className="h-4 w-4 mb-1" />
              <span className="text-xs">{item.label}</span>
              {item.highlight && currentPath !== item.path && <Volume className="h-3 w-3 text-netflix-red" />}
            </Link>)}
        </nav>
      </div>
    </header>;
