...
export const FloatingMenu = () => {
  ...
  return (
    <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-20 md:hidden">
      <nav className="flex items-center justify-center py-2 px-3 gap-1 bg-gradient-to-r from-gray-900/90 to-black/90 backdrop-blur-md border border-gray-800 rounded-full shadow-lg animate-slide-in-bottom">
        {menuItems.map((item) => (
          <Link
            key={item.path}
            to={item.path}
            className={cn(
              ...
              currentPath === item.path || currentPath.startsWith(item.path + '/')
                ? "text-netflix-red bg-gray-800/70 animate-pulse-soft"
